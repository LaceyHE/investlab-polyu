...
export function useScenarioSimulation(
  ...
  const setDateIndex = useCallback((index: number) => {
    ...
    setState(prev => {
      ...
      return { ...prev, currentDateIndex: clamped, currentNav: nav, navHistory: newHistory };
    });
  ...
  const updatePosition = useCallback((ticker: string, weight: number) => {
    setState(prev => {
      ...
      return { ...prev, positions: newPositions, cashWeight: 100 - newPositions.reduce((s, p) => s + p.weight, 0) };
    });
  }, [marketData, allDates]);
  ...
  useEffect(() => {
    if (isPlaying) {
      intervalRef.current = window.setInterval(() => {
        setState(prev => {
          if (prev.currentDateIndex >= allDates.length - 1) { setIsPlaying(false); return prev; }
          const next = prev.currentDateIndex + 1;
          const nav = prev.positions.length > 0 ? calculateNav(prev.positions, next) : prev.initialCapital;
          return { ...prev, currentDateIndex: next, currentNav: nav, navHistory: [...prev.navHistory, { date: allDates[next], value: nav }] };
        });
      }, 1000 / playSpeed);
    }
  ...
  const metrics: PortfolioMetrics = useMemo(() => {
    ...
    let peak = initial;
    let maxDD = 0;
    navHist.forEach(h => { peak = Math.max(peak, h.value); const dd = ((h.value - peak) / peak) * 100; maxDD = Math.min(maxDD, dd); });
    ...
    return { totalReturn, cagr, maxDrawdown: maxDD, worstQuarter: worstQ, sharpeRatio: sharpe, volatility: annualizedVol, netExposure: 100 - state.cashWeight };
  ...
  const rollingSharpe = useMemo(() => {
    const navHist = state.navHistory;
    if (navHist.length < 62) return [];
    const returns = navHist.slice(1).map((h, i) => (h.value - navHist[i].value) / navHist[i].value);
    const result: number[] = new Array(61).fill(0);
    for (let i = 60; i < returns.length; i++) {
      const window = returns.slice(i - 60, i);
      const avg = window.reduce((s, r) => s + r, 0) / window.length;
      const std = Math.sqrt(window.reduce((s, r) => s + (r - avg) ** 2, 0) / window.length);
      result.push(std > 0 ? (avg * 252) / (std * Math.sqrt(252)) : 0);
    }
    return result;
  }, [state.navHistory]);

  // Portfolio drawdown array
  const portfolioDrawdown = useMemo(() => {
    const navHist = state.navHistory;
    if (navHist.length < 2) return [];
    let peak = navHist[0].value;
    return navHist.map(h => { peak = Math.max(peak, h.value); return ((h.value - peak) / peak) * 100; });
  }, [state.navHistory]);

  const reset = useCallback(() => {
    setIsPlaying(false);
    setState({ currentDateIndex: 0, positions: [], cashWeight: 100, navHistory: [], tradeHistory: [], initialCapital: 1000000, currentNav: 1000000 });
  }, []);

  return { state, currentDate, metrics, isPlaying, playSpeed, setDateIndex, updatePosition, setIsPlaying, setPlaySpeed, reset, rollingSharpe, portfolioDrawdown };
}
